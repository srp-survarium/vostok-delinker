...
python3 build.py 04-synthetic-pdb
```
...
compiler's own -- `04-synthetic-pdb` demonstrates exactly that workflow, and is
therefore the one example that runs end-to-end with a period toolchain (its PDB
is synthesized in the modern format regardless of which MSVC linked the image;
validated with VC6 SP3 under Wine).
...
### 04-synthetic-pdb -- delink an image that shipped with no debug info

The reconstruction-project workflow in miniature: the image is linked release
(NO `/Zi`, no PDB), and the example's `synth_pdb.py` builds a PDB from the
project's own knowledge in two explicit stages. Stage 1 acquires the facts --
here the linker `.map` stands in for what a real project recovers with analysis
tooling -- and distills them into a symbol inventory (`example.symbols.tsv`):
...
Function RVAs come from the map's publics, sizes from the distance to the next
`.text` symbol, TU ownership from the map's object column. A real project
derives the same inventory from its own source annotations or analysis
databases and feeds it in directly via `--symbols`, skipping the map stage.

Stage 2 synthesizes the PDB from the inventory alone: per-function line
records under `c:\proj\` carry TU ownership, `S_GPROC32`/`S_LDATA32` module
records carry the names, `llvm-pdbutil yaml2pdb` produces the container, and
one byte-patch points the DBI's symbol-records field at an empty stream
(yaml2pdb leaves it nil, which the `pdb2` crate rejects):

```
[synth] 3 functions + 2 data symbols -> example.synth.pdb (module c:\proj\example.cpp)
reference functions: ['?compute@@YAHH@Z', '?helper@@YAHH@Z', '_main']
delinked  functions: ['?compute@@YAHH@Z', '?helper@@YAHH@Z', '_main']  <- same set
```

The image is linked `/NODEFAULTLIB /ENTRY:main`, so every byte belongs to the
example TU and the synthetic PDB's symbol coverage is COMPLETE. That mirrors
the real workflow's precondition: Vostok resolves data relocations against the
nearest named symbol at or below the target, so a reloc pointing below every
named symbol is fatal -- reconstruction projects name their whole image before
delinking, and so does this example.
