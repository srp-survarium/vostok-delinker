...
class Toolchain:
    ...
    def link_image(self, src: Path, out_exe: Path, mode: str, extra_link=()):
        # "reloc"/"fixed": /Zi builds exe + pdb; /FIXED (stripped) drops the
        # .reloc directory, /FIXED:NO keeps it. "release": NO debug info at all
        # (the 04-synthetic-pdb case) -- the only build product besides the exe
        # is a linker .map, standing in for the project's RE knowledge.
        # Run inside the output dir (see compile_object).
        debug = [] if mode == "release" else ["/Zi"]
        link = ["/FIXED" if mode == "fixed" else "/FIXED:NO", *extra_link]
        if mode == "release":
            link.append("/MAP:" + self.winpath(out_exe.with_suffix(".map")))
        _, log = run_compiler(self.cl + ["/nologo", *debug,
                              "/Fe" + self.winpath(out_exe),
                              "/Fo" + self.winpath(out_exe.with_suffix(".obj")),
                              self.winpath(src), "/link", *link], cwd=out_exe.parent)
        if mode == "release":
            if not out_exe.exists() or not out_exe.with_suffix(".map").exists():
                die("link failed:\n" + log)
            return None
...
def report(kind, ref: Path, got: Path, cfg):
    if kind == "functions":
        # Skip '.'-prefixed entries: section-ish labels (e.g. VC6's `.lf`),
        # not functions.
        fns = lambda o: sorted(
            n for l in objdump(o, "-d").splitlines() if ">:" in l
            if not (n := l.split("<")[1].split(">")[0]).startswith("."))
...
def main():
    ...
    pdb = tc.link_image(src, exe, mode=cfg["link"],
                        extra_link=cfg.get("link_flags", []))
    if cfg.get("pdb") == "synthetic":
        # No compiler PDB exists (release link). Synthesize one from the linker
        # map with the example-local synth_pdb.py -- the reconstruction-project
        # workflow in miniature (llvm-pdbutil yaml2pdb does the container work).
        print(f"  compiled reference {ref_obj.name}; linked {exe.name} "
              f"+ {exe.with_suffix('.map').name} (release: NO debug info)")
        pdb = build / (src.stem + ".synth.pdb")
        r = run([sys.executable, str(ex_dir / "synth_pdb.py"),
                 "--exe", str(exe), "--map", str(exe.with_suffix(".map")),
                 "--out", str(pdb), "--object", src.stem + ".obj",
                 "--source", cfg["source"]])
        if r.returncode != 0:
            die("synth_pdb failed:\n" + r.stdout + r.stderr)
    ...
    # 04-synthetic-pdb pins its own prefix instead (the synthetic module lives
    # under a fixed c:\proj\, not under the real source dir).
    engine = cfg.get("engine") or tc.winpath(src.parent) + ("\\" if not IS_WINDOWS else os.sep)
